...
setwd(paste0(homewd, "FigS1")) 

#load trees
Stree  <- read.tree(file = paste0(homewd,"FigS1/raxml-output/S/Sgene.raxml.supportFBP"))
Mtree  <- read.tree(file = paste0(homewd,"FigS1/raxml-output/M/Mgene.raxml.supportFBP"))
Etree  <- read.tree(file = paste0(homewd,"FigS1/raxml-output/E/Egene.raxml.supportFBP"))
Ntree  <- read.tree(file = paste0(homewd,"FigS1/raxml-output/N/Ngene.raxml.supportFBP"))
...
#load metadata
meta.dat <- read.csv(file = paste0(homewd,"FigS1/amino_acid_metadata_manual.csv"), header = T, stringsAsFactors = F)
head(meta.dat)
...
pS <- ggtree(Stree.root) %<+% Sdat + 
  geom_tippoint(aes(fill=subgroup, shape=bat_host), show.legend = F) +
  geom_nodelab(size=1.5,nudge_x = -.04, nudge_y = .7) +
...
pS1 <- cowplot::plot_grid(pAminoAcid, legall, nrow=2, ncol=1, rel_heights = c(1,.1))
