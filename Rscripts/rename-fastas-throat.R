...
all_names <- all_names[all_names!="rename-fastas-throat.R"]

#now make new names
new_names <- paste0(sapply(strsplit(all_names, split="_ts"), '[',1),".fasta")
...
rename.func <- function(old){
  ...
  new_slim = sapply(strsplit(old, split="_ts"), '[',1)
